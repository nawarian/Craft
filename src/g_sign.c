...
void g_sign_list_grow(SignList *list) {
    SignList new_list;
    g_sign_list_alloc(&new_list, list->capacity * 2);
...
void _g_sign_list_add(SignList *list, Sign *sign) {
    if (list->size == list->capacity) {
        g_sign_list_grow(list);
    }
...
{
    g_sign_list_remove(list, x, y, z, face);
    Sign sign;
    sign.x = x;
    sign.y = y;
    sign.z = z;
